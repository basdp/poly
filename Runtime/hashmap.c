...
int hashmap_get(map_t in, const char* key, any_t *arg){
	...
	for(i = 0; i< m->table_size; i++){
		if(m->data[curr].in_use && strcmp(m->data[curr].key, key) == 0 && m->data[curr].in_use == 1){
...
int hashmap_iterate(map_t in, PFany f, any_t item) {
	...
	for(i = 0; i< m->table_size; i++)
		if(m->data[i].in_use != 0) {
			...
			int status = f(item, data, m->data[i].key);
			if (status != MAP_OK) {
				//semaphore_V(m->lock);
				return status;
			}
...
void hashmap_print(map_t map) {
	printf("Hashmap:\n");
	hashmap_iterate(map, &hashmap_print_item, 0);
	printf("-------------\n\n");
}
