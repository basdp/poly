...
namespace SDILReader
{
    public class MethodBodyReader
    {
        ...
        {
            //ulong i = (ulong)(((il[position++] | (il[position++] << 8)) | (il[position++] << 0x10)) | (il[position++] << 0x18) | (il[position++] << 0x20) | (il[position++] << 0x28) | (il[position++] << 0x30) | (il[position++] << 0x38));
            double r = BitConverter.ToDouble(il, position);
            position += 8;
            return r;
        }
        ...
        private Single ReadSingle(byte[] _il, ref int position)
        {
            ...
            Single r = BitConverter.ToSingle(il, position);
            position += 4;
            return r;
        }
        #endregion
